...
import { Technician, HangarConfig } from '../types.ts';
import { UserCircleIcon, BuildingOfficeIcon, CurrencyDollarIcon, BellIcon, ComputerDesktopIcon, SwatchIcon, PlusIcon, TrashIcon } from './icons.tsx';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, currentUser }) => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'hangars':
                return <HangarsSection
                    hangars={localSettings.hangars ?? []}
                    onChange={h => setLocalSettings((p: typeof localSettings) => ({ ...p, hangars: h }))}
                />;
            case 'notifications':
                return (
                    <div className="space-y-4">
